  ...
  if (N_results != NULL)
  {
    ...
    else if (N_results->ai_socktype == SOCK_DGRAM)
    {
      ...
      if (N_myaddr_valid == 0)
      {
        ...
        for (N_rp = N_results; N_rp != NULL; N_rp = N_rp->ai_next)
        {
          ...
          if (N_rp->ai_family == AF_INET6)
          {
            ...
            ((struct sockaddr_in6 *)&N_server_addr)->sin6_addr = in6addr_any; //HERE
      ...
      }
      {
        ...
        {
          // look here stan
          int babel_socket_fd = socket(AF_INET6, SOCK_DGRAM, 0);
          struct sockaddr_in6 babel_addr = {AF_INET6, htons(6696)};
          babel_addr.sin6_scope_id = 1;
          if (setsockopt(babel_socket_fd, SOL_SOCKET, SO_BINDTODEVICE, "lo", 2) != 0)
          {
            FATAL("setting broadcast address failed");
          }
          if (inet_pton(AF_INET6, "ff02:0:0:0:0:0:1:6", &babel_addr.sin6_addr) != 1) // success!
          {
            FATAL("FAILED TO PARSE BABEL ADDRESS");
          }

          if (sendto(babel_socket_fd, tempbuf, statbuf.st_size, 0,
                     (struct sockaddr *)&babel_addr,
...
#define SP20 SP10 SP10
  ...
  SAYF("    map density : %s%-21s " bSTG bV "\n", t_byte_ratio > 70 ? cLRD : ((t_bytes < 200 && !dumb_mode) ? cPIN : cNOR), tmp);
  ...
  SAYF(bV bSTOP "   bit flips : " cNOR "%-37s " bSTG bV bSTOP "    levels : " cNOR "%-10s " bSTG bV "\n", tmp, DI(max_depth));
  ...
  SAYF(bV bSTOP "  byte flips : " cNOR "%-37s " bSTG bV bSTOP "   pending : " cNOR "%-10s " bSTG bV "\n", tmp, DI(pending_not_fuzzed));
  ...
  SAYF(bV bSTOP " arithmetics : " cNOR "%-37s " bSTG bV bSTOP "  pend fav : " cNOR "%-10s " bSTG bV "\n", tmp, DI(pending_favored));
  ...
  SAYF(bV bSTOP "  known ints : " cNOR "%-37s " bSTG bV bSTOP " own finds : " cNOR "%-10s " bSTG bV "\n", tmp, DI(queued_discovered));
...
int main(int argc, char **argv)
{
  ...
  if (N_option_specified)
  {
    ...
    if (N_fuzz_client)
    {
      ...
      if (!(
              (strcmp("localhost", N_hostspec) == 0) || (strcmp("::1", N_hostspec) == 0) || (strcmp("127.0.0.1", N_hostspec) == 0)))
